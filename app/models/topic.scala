...
object Topic extends SQLSyntaxSupport[Topic] {
  ...
  def create(name: String)(implicit session: DBSession = autoSession): Topic = {
    val id = withSQL {
  ...
  def find(id: Long)(implicit session: DBSession = autoSession): Option[Topic] = {
    withSQL { select.from(Topic as t).where.eq(t.id, id) }
      .map(Topic(t.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[Topic] = {
